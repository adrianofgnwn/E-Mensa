    ...
    public function verifylogin(RequestData $request) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') { // Use POST (More Secure)
            ...
            if (db_increment_login_count($user['id'], $link) && db_update_last_login($user['id'], $link)) {
                ...
                // Check for redirect_after_login and redirect appropriately
                if (isset($_SESSION['redirect_after_login'])) {
                    $redirectUrl = $_SESSION['redirect_after_login']; // Retrieve the intended redirect URL
                    unset($_SESSION['redirect_after_login']); // Clear it after using
                    return $this->redirect($redirectUrl); // Redirect to the intended page
                }

                // Default redirect to home page
                $_SESSION['login_success'] = 'Login successful! Welcome back, ' . htmlspecialchars($user['name']) . '.';
                return $this->redirect('/emensa');
            } else {
                mysqli_rollback($link);
