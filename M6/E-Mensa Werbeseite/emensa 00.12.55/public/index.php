...
//These lines are importing the Monolog classes you'll use:
use Monolog\Logger; //This is the main logging class that will handle log messages.
use Monolog\Handler\StreamHandler; //This specifies where the logs should be written (e.g., a file, console, or another output stream).

//Logger::DEBUG: Sets the logging level, meaning that all messages with a priority of DEBUG and above (like INFO, ERROR, etc.) will be logged.

assert_php_version('8.2.0');
assert_path();

$log = new Logger('my_app');
$log->pushHandler(new StreamHandler(__DIR__ . '/../logs/app.log', Logger::DEBUG)); // StreamHandler: Defines how and where log messages will be stored.
...
function logger() {
    static $logger = null; //This ensures the $logger variable is shared across all calls to this function.


    if ($logger === null) {
        $logger = new Logger('e-mensa'); // Create a new logger with the name 'e-mensa'

        // Specify the log file location
        $logFile = __DIR__ . '/../storage/logs/app.log';

        // Tell Monolog to write logs into this file
